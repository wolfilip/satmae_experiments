...
def build_terrammind_vit(
    ...
    **kwargs,
    ...
    model = TerraMindViT(pretrained=pretrained, in_chans=num_channels, **kwargs)
...
def build_terrammind_tim(
    ...
    **kwargs,
    ...
    if ckpt_path is not None:
        ...
        warnings.warn(
            "TerraMind TiM model not pre-trained. Generation of TiM modalities will not work correctly."
        )
...
def terramind_v01_base(**kwargs):
    if kwargs.get("pretrained", False):
        if not os.getenv("HF_TOKEN", None):
            warnings.warn(
                "TerraMind v0.1 models require a HF_TOKEN with access to model weights."
            )
