...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.head = nn.Conv2d(256, self.num_classes, kernel_size=3, padding=1)
        # self.head = (
        ...
        self.upernet_head = UperNetHead(config, [256, 512, 1024, 1024])

        # self.PPN = PSPModule(1024)
        # self.FPN = FPN_fuse([256, 512, 1024, 1024], fpn_out=512)

        self.num_patches = [
            int(self.img_size / 8),
            int(self.img_size / 16),
            int(self.img_size / 32),
            int(self.img_size / 32),
        ]
    ...
    def decoder_upernet(self, features, conv_embeds):
        ...
        new_features.append(features[0].reshape(-1, 28, 28, 256))
        new_features.append(features[1].reshape(-1, 14, 14, 512))
        new_features.append(features[2].reshape(-1, 7, 7, 1024))
        new_features.append(features[3].reshape(-1, 7, 7, 1024))
    ...
    def forward(self, x):
        ...
        new_features.append(
            features[0].reshape(-1, self.num_patches[0], self.num_patches[0], 256)
        )
        new_features.append(
            features[1].reshape(-1, self.num_patches[1], self.num_patches[1], 512)
        )
        new_features.append(
            features[2].reshape(-1, self.num_patches[2], self.num_patches[2], 1024)
        )
        new_features.append(
            features[3].reshape(-1, self.num_patches[3], self.num_patches[3], 1024)
        )
