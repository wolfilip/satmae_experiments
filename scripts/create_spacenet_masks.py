...
def create_masks(
    ...
    path_masks_plot = os.path.join(path_data, "masks_" + str(buffer_meters) + "m_plots")
    # image directories
    ...
    for i, im_name in enumerate(im_files):
        ...
        if not os.path.exists(mask_file) or overwrite_ims:
            mask, gdf_buffer = apls_utils.get_road_buffer(
                label_file_tot,
                im_file_out_vis,
                mask_file,
...
def main():
    ...
    parser.add_argument(
        "--n_bands", default=3, type=int, help="Number of bands to use [3,8]"
    )
    parser.add_argument(
        "--burnValue", default=150, type=int, help="Value of road pixels (for plotting)"
    )
    parser.add_argument(
        "--make_plots",
        default=1,
    ...
    output_df_file = os.path.join(
        args.output_df_path,
        data_root + "_" + "files_loc_" + str(args.buffer_meters) + "m.csv",
    )

    path_masks = create_masks(
        args.path_data,
